...
def top_supporters(response: Response, limit: Limit = 10, session: Session = Depends(get_session)):
    """Ko-fi supporters ranked by their total donated amount (amounts are not exposed)."""
    ...
    rows = session.execute(
        select(first_name)
        .where(*visible, Supporter.source == SupporterSource.kofi)
        .group_by(func.lower(func.trim(Supporter.name)))
        .order_by(total.desc(), first_at)
        .limit(limit)
    ).scalars()
    response.headers["Cache-Control"] = CACHE_CONTROL
    return [TopSupporter(rank=i, name=name.strip()) for i, name in enumerate(rows, start=1)]
...
def recent_donations(response: Response, limit: Limit = 10, session: Session = Depends(get_session)):
    """Most recent Ko-fi donations, newest first."""
    rows = session.scalars(
        select(Supporter)
        .where(*visible, Supporter.source == SupporterSource.kofi)
